...
import thw.rootsproj.ApplicationMain;
...
public class SceneBuilder {
	...
	private RootSystemProjector projector;
	...
	private void buildResetButton(VBox controlBox) {
		...
		this.resetButton.setOnAction(event -> {
			this.spinners.forEach(s -> s.getValueFactory().setValue(0));
		});
	...
	private void buildSpinners(VBox controlBox) {
		...
		sb.append("Keyboard ARROW-UP: Increment by 1");
		sb.append(newL);
		sb.append("Keyboard ARROW-DOWN: Decrement by 1");
		sb.append(newL);
		sb.append("Keyboard ARROW-RIGHT: Increment by ");
		sb.append(Constants.SPINNER_CHUNK);
		sb.append(newL);
		sb.append("Keyboard ARROW-LEFT: Decrement by ");
		sb.append(Constants.SPINNER_CHUNK);
	...
	private void triggerCanvasPaint() {
		int[] planeParams = this.spinners.stream().mapToInt(s -> s.getValue()).toArray();
	...
	private void controlsOnMovieStart() {
		...
		this.spinners.stream().forEach(s -> s.setDisable(true));
	...
	public void controlsOnMovieStopped() {
		...
		this.stop.setDisable(true);
		this.spinners.stream().forEach(s -> s.setDisable(false));
